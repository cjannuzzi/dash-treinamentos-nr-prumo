...
st.markdown("""
    <style>
        ...
        [data-testid="stSidebar"] { background-color: #002B49; }
        [data-testid="stSidebar"] * { color: #ffffff !important; }
        [data-testid="stSidebar"] input { color: #002B49 !important; }
        ...
        div[data-testid="stMetricLabel"] { font-size: 0.9rem !important; color: #6C757D !important; white-space: normal !important; }
        div[data-testid="stMetricValue"] { font-size: 1.5rem !important; color: #002B49 !important; font-weight: 700 !important; }
        div[data-testid="stMetricDelta"] { font-size: 0.85rem !important; margin-top: 5px; white-space: normal !important; }
        div[data-testid="stMetricDelta"] svg { display: none; }
        h1, h2, h3 { color: #002B49; font-family: 'Segoe UI', sans-serif; }
    </style>
...
if df is not None:
    # --- SIDEBAR ---
    st.sidebar.image("https://img.icons8.com/fluency-systems-regular/96/ffffff/search.png", width=40)
    ...
    st.title("📊 Painel Gerencial QSSMA")
    st.divider()
    ...
    # --- SEÇÃO 1: GRÁFICOS ORIGINAIS (Mantidos) ---
    ...
    with col_orig1:
        ...
        if not df_ext.empty:
            ...
            fig.update_traces(marker_color='#002B49', textfont_color='white')
    ...
    with col_orig2:
        ...
        cores = {'Externo (Custo)': '#002B49', 'Interno (SESMT)': '#00A3E0', 'Não Aplicável (N/A)': '#D1D5DB'}
        fig2 = px.pie(df_pie, values='Qtd', names='Status', hole=0.6, color='Status', color_discrete_map=cores)
        st.plotly_chart(fig2, use_container_width=True)

    st.divider()

    # --- SEÇÃO 2: NOVOS GRÁFICOS (Inteligência Competitiva) ---
    st.markdown("### 📈 Análise Comparativa & Rankings")
    ...
    # 3 Colunas para os Novos Gráficos
    ...
    with g_new1:
        ...
        if not df_rank_ext.empty:
            ...
            fig_r1.update_traces(marker_color='#E63946', textfont_color='white') # Vermelho para Custo
    ...
    with g_new2:
        ...
        if not df_rank_int.empty:
            ...
            fig_r2.update_traces(marker_color='#2A9D8F', textfont_color='white') # Verde para Saving
    ...
    with g_new3:
        ...
        # Gráfico Empilhado para ver proporção
        df_prop = df_filtered.groupby([agrupar, 'Status']).size().reset_index(name='Qtd')
        # Filtra Top 10 volumes para não poluir
        ...
        cores_prop = {'Externo (Custo)': '#E63946', 'Interno (SESMT)': '#2A9D8F'}
        fig_r3 = px.bar(df_prop, x='Qtd', y=agrupar, color='Status', orientation='h', 
                        color_discrete_map=cores_prop, text='Qtd')
        fig_r3.update_layout(xaxis_title="Quantidade", yaxis_title=None, plot_bgcolor='rgba(0,0,0,0)', 
